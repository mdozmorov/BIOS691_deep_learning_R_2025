...
This notebook contains the code samples found in Chapter 2, Section 2.5 of [Deep Learning with R](https://www.manning.com/books/deep-learning-with-r-second-edition). Note that the original text features far more content, in particular further explanations and figures: in this notebook, you will only find source code and related comments.
...
```{r}
# Take first 10 test images and get model predictions
test_digits <- test_images[1:10, ]
predictions <- predict(network, test_digits)
str(predictions)  # Show structure of predictions
predictions[1, ]  # Show probabilities for first test image

which.max(predictions[1, ])  # Get the digit with highest probability
predictions[1, 8]  # Show probability for digit 7 (index 8)

test_labels[1]  # Show actual label for first test image
```
